 ...
 **/
cPacket* BaseMacLayer::decapsMsg(MacFrame* msg)
{
 ...
 * header fields.
 **/
MacFrame* BaseMacLayer::encapsMsg(cPacket *netwPkt)
{
    MacFrame *pkt = new MacFrame(netwPkt->getName(), netwPkt->getKind());
...
{
    MacFrame*          mac  = static_cast<MacFrame *>(msg);
...
Mapping* BaseMacLayer::createRectangleMapping(simtime_t_cref start, simtime_t_cref end, Argument::mapped_type_cref value)
{
	...
	//set position Argument
	Argument preStartPos(MappingUtils::pre(start));
	//set discontinuity at position
	MappingUtils::addDiscontinuity(m, preStartPos, Argument::MappedZero, start, value);

	//set position Argument
	Argument postEndPos(MappingUtils::post(end));
	//set discontinuity at position
	MappingUtils::addDiscontinuity(m, postEndPos, Argument::MappedZero, end, value);
...
const LAddress::L2Type& BaseMacLayer::getUpperDestinationFromControlInfo(const cObject *const pCtrlInfo) {
	return NetwToMacInfo::getDestinationFromInfo(pCtrlInfo);
}
